...
+ nic-k8s-01：172.27.138.251
+ nic-k8s-02：172.27.137.229
+ nic-k8s-03：172.27.138.239
...
``` bash
hostnamectl set-hostname nic-k8s-01 # 将 nic-k8s-01 替换为当前主机名
```
...
``` bash
cat >> /etc/hosts <<EOF
172.27.138.251 nic-k8s-01
172.27.137.229 nic-k8s-02
172.27.138.239 nic-k8s-03
EOF
```
...
本操作只需要在 nic-k8s-01 节点上进行，设置 root 账户可以无密码登录**所有节点**：

``` bash
ssh-keygen -t rsa 
ssh-copy-id root@nic-k8s-01
ssh-copy-id root@nic-k8s-02
ssh-copy-id root@nic-k8s-03
```
